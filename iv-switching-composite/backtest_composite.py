    ...
    for i, date in enumerate(trading_days):
        ...
        if mode in ['A', 'B']:
            ...
            if d3_active and port.peak_vix and port.peak_vix > 30 and len(port.open_zebras) < 2:
                ...
                if net_debit > 0:
                    # D3: same 15% slot sizing as Mode B — no runaway with NAV growth
                    cnt = size_zebra_trade(nav, net_debit, n_open=len(port.open_zebras))
                    # Hard dollar cap: max $3,750 per slot (15% of original $25K)
                    max_contracts = max(int(INITIAL_CAPITAL * 0.15 / (net_debit * 100)), 1)
                    cnt = min(cnt, max_contracts)
