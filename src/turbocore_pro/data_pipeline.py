...
class TurboCoreDataPipeline:
    ...
    def prepare_core_features(self) -> pd.DataFrame:
        ...
        if '^VIX3M' in self.data and '^VIX' in self.data:
            ...
            master['vix_term_slope'] = (raw_vts - raw_vts.rolling(63).mean()) / raw_vts.rolling(63).std()
            master['vix_term_slope'] = master['vix_term_slope'].fillna(0.0)
        else:
            master['vix_term_slope'] = 0.0
            
        if 'HYG' in self.data:
            master['hyg_close'] = self.data['HYG']['Close'].reindex(master.index).ffill()
            hyg_zscore = (master['hyg_close'] - master['hyg_close'].rolling(60).mean()) / master['hyg_close'].rolling(60).std()
