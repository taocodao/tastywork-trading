"""
SNDK Dynamic Ladder Strategy - Ladder Manager
===========================================
...
class LadderManager:
    ...
    def convert_to_spread_if_needed(self, rung: LadderRung, ml_confidence: float, spot: float, T: float, iv: float) -> LadderRung:
        ...
        if ml_confidence >= self.config.spread_conversion_conf_low:
            ...
            wing_delta = get_bs_delta(spot, wing_strike, T, 0.045, iv, opt=rung.opt_type)
            wing_iv_adj = _skew_adjusted_iv(iv, wing_delta, option_type=rung.opt_type)
            wing_premium = get_bs_price(spot, wing_strike, T, 0.045, wing_iv_adj, opt=rung.opt_type)
    ...
    def manage_positions(self, today: pd.Timestamp, spot: float, iv_current: float) -> Tuple[List[dict], float]:
        ...
        for rung_list in [self.call_rungs, self.put_rungs]:
            ...
            for rung in rung_list:
                ...
                # First get current delta using unadjusted IV to know how much to adjust IV
                current_delta_raw = get_bs_delta(spot, rung.strike, T_rem, 0.045, iv_current, opt=rung.opt_type)
                iv_adj = _skew_adjusted_iv(iv_current, current_delta_raw, option_type=rung.opt_type)
                
                # Mark to market
                current_prem = get_bs_price(spot, rung.strike, T_rem, 0.045, iv_adj, opt=rung.opt_type)
                if rung.is_spread:
                    wing_delta_raw = get_bs_delta(spot, rung.wing_strike, T_rem, 0.045, iv_current, opt=rung.opt_type)
                    wing_iv_adj = _skew_adjusted_iv(iv_current, wing_delta_raw, option_type=rung.opt_type)
                    current_wing = get_bs_price(spot, rung.wing_strike, T_rem, 0.045, wing_iv_adj, opt=rung.opt_type)
                ...
                current_delta = abs(get_bs_delta(spot, rung.strike, T_rem, 0.045, iv_adj, opt=rung.opt_type))
