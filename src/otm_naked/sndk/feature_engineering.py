"""
SNDK Dynamic Ladder Strategy - Feature Engineering
================================================
Extends the base HILO-IV feature engineering with SNDK-specific features:
- daily_move_pct
- gap_pct
- daily_range_pct
- atr_14
...
def _skew_adjusted_iv(atm_iv, delta, option_type="put"):
    """Adjust IV for skew: OTM puts trade at higher IV than flat surface.
    Based on Perplexity research: ~8 vol pts per 10-delta OTM for puts on high-beta stocks.
    """
    skew_rate = 0.08  # 8 vol pts per 10-delta OTM step for puts
    if option_type == "put":
        otm_pct = max(0, 0.50 - abs(delta)) / 0.10  # How far OTM in delta steps
        return atm_iv + skew_rate * otm_pct
    else:  # call — slightly discounted vs flat surface
        otm_pct = max(0, abs(delta) - 0.50) / 0.10
        return atm_iv - 0.03 * otm_pct

def _classify_regime(adx, slope, ema_cross_up, hurst, roc_5d=0.0):
    """Classify market regime from indicators.
    Now includes ROC5 hard block per Perplexity recommendation:
    ROC5 > 15% forces EXTREME_UPTREND regardless of ADX lag.
    """
    # ROC5 hard block: patches the 14-bar ADX lag vulnerability
    if roc_5d > 15.0:
...
def build_sndk_features(
    ...
    """
    Builds the base HILO features and appends the 6 SNDK-specific features.
    """
    # 1. Get base features
