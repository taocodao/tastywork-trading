"""
SNDK Dynamic Ladder Strategy - Optuna Optimizer
===============================================
...
def run_optuna_optimization(backtest_engine, df_train: pd.DataFrame, n_trials: int = 100) -> dict:
    ...
    def objective(trial):
        ...
        profit_pct   = trial.suggest_categorical("profit_take_pct", [0.20, 0.30, 0.40]) # Lower targets to recycle capital faster
        profit_short = trial.suggest_categorical("profit_take_pct_short", [0.15, 0.20, 0.25])
        sl_mult      = trial.suggest_categorical("stop_loss_credit_mult", [3.0, 4.0, 5.0])
        trigger_pct  = trial.suggest_categorical("entry_trigger_pct", [0.5, 1.0, 1.5])  # Small dips
        ivr_min      = trial.suggest_categorical("ivr_min", [20, 30, 40])  # Lower IVR gates for more frequent entries
