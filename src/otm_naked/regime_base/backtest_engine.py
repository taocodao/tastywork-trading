"""
RegimeBase Dynamic Ladder Strategy - Walk-Forward Backtest Engine
===========================================================
Executes the walk-forward optimization protocol for RegimeBase.
"""
import logging
import math
import pandas as pd
...
from src.otm_naked.regime_base.ladder_manager import LadderManager, LadderRung, _calc_friction
from src.otm_naked.regime_base.optuna_optimizer import run_optuna_optimization
...
class RegimeBaseBacktestEngine:
    def __init__(self, config: RegimeBaseLadderConfig):
        self.config = config
    ...
    def create_labels(self, df: pd.DataFrame, dte_at_entry: int, profit_target: float) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            ...
            if opt == "put":
                ...
                strike = find_put_strike(S, T, 0.045, iv, target_delta=base_delta)
            else:
                from src.otm_naked.strike_selector import find_call_strike
                strike = find_call_strike(S, T, 0.045, iv, target_delta=base_delta)
            
            entry_prem = get_bs_price(S, strike, T, 0.045, iv, opt=opt)
            ...
            for j in range(1, 31):
                ...
                current_prem = get_bs_price(S_future, strike, T_remaining, 0.045, iv_future, opt=opt)
                if entry_prem > 0:
    ...
    def simulate_strategy(self, df: pd.DataFrame, use_ml: bool = True, ml_model=None, ml_features=None) -> list:
        ...
        signal_engine = RegimeBaseLadderSignalEngine(self.config)
        ...
        for date, row in df.iterrows():
            ...
            new_rung = LadderRung(
                ...
                entry_delta=abs(get_bs_delta(S, strike, T_new, 0.045, iv, opt=signal.direction)),
                entry_iv=iv,
                entry_date=date,
            ...
            # Spread conversion
            new_rung = ladder.convert_to_spread_if_needed(new_rung, ml_confidence, S, T_new, iv)
        ...
        if len(df) > 0:
            ...
            for rung_list in [ladder.call_rungs, ladder.put_rungs]:
                for rung in rung_list:
                    ...
                    current_prem = get_bs_price(S, rung.strike, T_rem, 0.045, iv, opt=rung.opt_type)
                    if rung.is_spread:
                        wing_prem = get_bs_price(S, rung.wing_strike, T_rem, 0.045, iv, opt=rung.opt_type)
