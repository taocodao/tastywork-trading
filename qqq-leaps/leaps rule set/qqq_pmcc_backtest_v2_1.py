...
def run_backtest(df: pd.DataFrame,
    ...
    for idx, row in df.iterrows():
        ...
        gap_pct = (S_open - row.get("prev_close", S_open)) / row.get("prev_close", S_open)                   if "prev_close" in row else 0.0
        ...
        if pos.leaps_status == LeapsStatus.NONE and positions_open < max_positions:
            ...
            if regime in [Regime.BULL_STRONG, Regime.BULL_MODERATE, Regime.CHOPPY]                and ml_conf >= threshold and is_gap:
        ...
        lm = bs_call_price(S, pos.leaps_strike, T_l2, r, sigma) * 100 * pos.leaps_contracts              if pos.leaps_status == LeapsStatus.OPEN else 0
        sm = bs_call_price(S, pos.short_strike, T_s2, r, sigma) * 100 * pos.leaps_contracts              if pos.pmcc_status != PmccStatus.NONE else 0
        nav = cash + lm - sm
