...
import os
# Re-save the system code
system_code = open("/dev/stdin").read() if False else None
        ...
        for rung_list in [call_rungs, put_rungs]:
            ...
            for rung in rung_list:
                ...
                if mtm["pnl_pct"] >= profit_pct:
                    ...
                    roll_credit = bs_price(S, rung["strike"], T_rem+30/365, iv, opt=rung["opt"]) - \
                                  bs_price(S, rung["strike"], T_rem, iv, opt=rung["opt"])
...
with open("output/sndk_full_system.py", "w") as f:
    f.write(system_code)

lines = len(system_code.split('\n'))
print(f"Saved {lines} lines to output/sndk_full_system.py")
