...
class DiagonalLiveScanner:
    def __init__(self):
        try:
            ...
            logger.info("DiagonalLiveScanner: ML signal enhancer loaded.")
        except Exception as e:
            logger.warning(f"DiagonalLiveScanner: ML enhancer not available ({e}) — falling back to rule-based.")
            self.ml_enhancer = None

        self.ta_engine    = TASignalEngine(ml_model=None, ml_enhancer=self.ml_enhancer)
    ...
    def scan(self) -> int:
        ...
        try:
            ...
            if action == 'OPEN_DIAGONAL':
                ...
                ml_boost_str = ""
                if self.ml_enhancer and hasattr(self.ta_engine, '_last_df') and self.ta_engine._last_df is not None:
                    enhanced = self.ml_enhancer.enhance_dip_score(self.ta_engine._last_df, 0.0)
                    ml_boost_str = f", ML boost={enhanced.ml_boost:+.2f}"

                logger.info(f"  ✅ Diagonal Signal FOUND: Regime={regime}, Dip={dip_score:.2f}{ml_boost_str}, ML={ml_pred['direction']} ({ml_pred['confidence']:.2f})")
