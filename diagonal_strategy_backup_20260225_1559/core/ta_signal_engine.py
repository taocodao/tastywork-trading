...
class TASignalEngine:
    def __init__(self, ml_model=None, ml_enhancer=None):
        self.ml_model    = ml_model    # XGBoost oscillation predictor (optional)
        self.ml_enhancer = ml_enhancer # MLSignalEnhancer (optional)
        self._last_df: Optional[pd.DataFrame] = None  # cached for ML enhancer
    ...
    def compute_features(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        df = df.copy()
        self._last_df = df  # Store copy for ML enhancer use
    ...
    def dip_score(self, features: Dict[str, Any]) -> float:
        ...
        base = self._rule_based_dip_score(features)
        if self.ml_enhancer is not None and self._last_df is not None:
            enhanced = self.ml_enhancer.enhance_dip_score(self._last_df, base)
    ...
    def bounce_score(self, features: Dict[str, Any]) -> float:
        ...
        base = self._rule_based_bounce_score(features)
        if self.ml_enhancer is not None and self._last_df is not None:
            enhanced = self.ml_enhancer.enhance_bounce_score(self._last_df, base)
            return enhanced.final_score
        return base
