...
class ActiveDiagonalManager:
    ...
    def _evaluate_idle(self, pos, ta, ml, mkt) -> str:
        ...
        # VIX spike entry bonus: when fear spikes sharply, premiums are very rich
        # Lower threshold by 5% to capture those peak-fear entries
        vix_spike_threshold = getattr(self.config, 'TA_VIX_SPIKE_ENTRY_BONUS', 3.0)
        if mkt.get('vix_change_1d', 0) > vix_spike_threshold:
            threshold *= 0.95  # 5% easier entry during VIX spike
            logger.debug(f"IDLE: VIX spike {mkt['vix_change_1d']:.1f}pts — lowering entry threshold to {threshold:.3f}")
    ...
    def _evaluate_full_diagonal(self, pos, ta, ml, mkt) -> str:
        ...
        # Priority 3.5: SCALE-IN on big dip (threshold tightened -20% → -25% to reduce drawdown)
        # BLOCKED when crash guard is active (HIGH_VOL BEARISH) to prevent averaging into a crash.
        ...
        if (anchor_pnl_pct <= -0.25
                and not getattr(pos, 'scale_in_used', False)
                and not crash_guard_active):
            dip_score = self.ta_engine.dip_score(ta)
            if dip_score > self.config.TA_DIP_SCORE_THRESHOLD * 1.15:
                logger.info(
                    f"FULL_DIAG: Anchor at {anchor_pnl_pct:.1%} loss + dip={dip_score:.2f} — SCALE IN"
                )
                return 'SCALE_IN'
        ...
        # Priority 5: ML Trend Speed exit stage (check before bounce score for earlier exits)
        if hasattr(self.ta_engine, 'ml_enhancer') and self.ta_engine.ml_enhancer is not None:
            ts_bars = mkt.get('tqqq_bars')
            if ts_bars is not None:
                exit_stage = self.ta_engine.ml_enhancer.get_exit_stage(ts_bars)
                from diagonal_strategy.indicators.trend_speed import ExitStage
                if exit_stage == ExitStage.CONFIRMATION:
                    hedge_pnl_pct = self._hedge_pnl_pct(pos, mkt)
                    if hedge_pnl_pct > 0:  # Only close if hedge has any profit
                        logger.info("FULL_DIAG: TrendSpeed CONFIRMATION — closing hedge early.")
                        return 'CLOSE_HEDGE'
                elif exit_stage == ExitStage.EARLY_WARNING:
                    logger.info("FULL_DIAG: TrendSpeed EARLY_WARNING — monitoring for close.")

        # Priority 6: Bounce detected -> close hedge cheap
